...
export default function SignupPage() {
    ...
    function handleAddFields(){
     ...
     function handleSubmit(e){
        ...
        const passwordConfirm = form.get("passwordConfirmation");
        const companyFlag = form.get("companyFlag");
        signup({name, email, password, passwordConfirm, companyFlag, fields});
    }
    return (<section className="bg-[#F4F7FF] py-14 lg:py-[90px] dark:bg-dark">
                        ...
                        <div
                            ...
                            <div className="mb-10 text-center">
                                <a href="javascript:void(0)" className="mx-auto inline-block max-w-[160px]">
                                    <img src={LOGOS.logo} alt="logo" className="dark:hidden"/>
                                    <img src={LOGOS.logoWhite} alt="logo"
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="mb-9">
                                    <label className="flex items-center">
                                        <input type="checkbox" checked={companyOwner} onChange={() => setCompanyOwner(prev => !prev)} className="mr-3"/>
                                        <span className="text-base text-body-secondary">I am a company owner</span>
                                    </label>
                                </div>
                                {companyOwner &&(
                                    <>
                                        {/*add the fields functionality */}
                                        <div className="mb-9 relative">
                                            <input name={"fields"} type="text" placeholder="Fields" value={field}
                                                   onChange={(e) => setField(e.target.value)}
                                                   className="w-full px-5 py-3 text-base transition bg-transparent border rounded-md outline-none border-stroke dark:border-dark-3 text-body-color dark:text-dark-6 placeholder:text-dark-6 focus:border-primary dark:focus:border-primary focus-visible:shadow-none"/>
                                            <span
                                                className="absolute py-1 px-2 text-white bg-primary rounded-full right-3 top-2 pointer"
                                                onClick={handleAddFields}>+</span>
                                        </div>

                                        <div className="mb-9 flex flex-wrap -mx-2">
                                            {fields.map((field, index) => (
                                                <div className=" mb-2 w-1/2 px-2 py-2">
                                                    <div
                                                        className="bg-primary text-white  rounded-full  relative">
                                                        <span key={index}>{field}</span>
                                                        <span
                                                            onClick={() => setFields(prev => prev.filter(f => f !== field))}
                                                            className={"cursor-pointer bg-pink-600 absolute top-3 right-2 px-1 text-xl w-5 h-1 block"}
                                                        ></span>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                        <input type="hidden" name="companyFlag" value={1}/>
                                    </>
                                )
                                }
                                <div className="mb-9">
                            ...
                            <p className="text-base text-body-secondary">
                                ...
                                <Link to="signin.html" className="text-primary hover:underline">
                                    Sign In
                                </Link>
                            </p>
